...
namespace nopenotdark\replaysystem\hotbar;
...
class HotbarManager {
    ...
    public function __construct(ReplaySystem $plugin) {
        $plugin->getServer()->getPluginManager()->registerEvents(new HotbarListener(), $plugin);
    }

    public function clearHotbar(Player $player): void {
    ...
    public function setHotbar(Player $player, AbstractHotbar $hotbar): void {
        $this->clearHotbar($player);

        foreach ($hotbar->getItems() as $index => $item) {
